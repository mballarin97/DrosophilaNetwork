...
def map_roi_to_number(links):
    def find_attr(roi):
        for key, value in roi_numerical.items():
            if roi == key:
                return value
    res = list(map(find_attr, links))
    return res
...
# temp['superset_number'].astype(int, copy=False)
temp['roi_number'] = map_roi_to_number(roi_dataframe['roi'].tolist())
```

```{python}
colors = ['#7f52ab', '#67e682', '#a3e443', '#f3747e', '#8095cc', '#452664', 
...
plt.matshow(nx.adjacency_matrix(graph).todense(), fignum=matfig.number)
```
