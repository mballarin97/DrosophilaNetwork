...
print(len(synapses_dataframe))
print(synapses_dataframe[synapses_dataframe['synaps'] == (200326126, 295816140)])
```
...
cols = list(roi_dataframe.columns)
print(cols)
print(roi_dataframe.head())
print(roi_dataframe['synaps'].nunique())
print(len(roi_dataframe))
print(4218945 - 3521163)
print(roi_dataframe[roi_dataframe['synaps'] == (200326126, 295816140)])
```
...
```{python}
roi_dataframe = pd.DataFrame(roi, columns=cols)
roi_dataframe.head()
...
```{python}
print(sorted(roi_dataframe['roi'].unique() ))
print((roi_dataframe['roi'].nunique()))
print(len(roi_dataframe))
```
...
```{python}
# sanity check
roi = list(sorted(roi_dataframe['roi'].unique()))
removed = []
for elem in roi:
    for key, value in roi_supersets.items():
...
attr_dict = nodes.set_index('bodyId')['type'].to_dict()
```

```{python}
graph = nx.Graph()
graph.add_nodes_from(neurons_dataframe['bodyId'])
nx.set_node_attributes(graph, attr_dict, 'type')
