...
class ClusterGrainer():
    def __init__(self, graph, n_clusters, preserve_roi=False):
        self.G = graph
    ...
    def zip_graph(self, partitions, inplace=True):
        if inplace:
            if(self.preserve_roi):
                self.G = my_induced_graph(partitions, self.G)
            else:
            ...
            if self.preserve_roi:
                return my_induced_graph(partitions, self.G)
            else:
...
Grainer = ClusterGrainer(g, 10000, preserve_roi=True)
```
...
```{python}
start = time.time()
Grainer.experiment([clusters[5]], metric)
end = time.time()
