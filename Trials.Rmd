...
```{python}
def my_induced_graph(partition, graph):
    ...
    for node1, node2, datas in graph.edges(data=True):
        ...
        edge_superset = datas.get('superset', [])
        edge_roi = datas.get('roi', [])
        com1 = partition[node1]
        com2 = partition[node2]
        w_prec = ret.get_edge_data(com1, com2, {'weight': 0}).get('weight', 1)
#        roi_prec = ret.get_edge_data(com1, com2, {'roi': []}).get('roi', [])
        superset_prec = ret.get_edge_data(com1, com2, {'superset': []}).get('superset', [])
        ret.add_edge(com1, com2, **{'weight': w_prec + edge_weight,
                                    'superset': superset_prec + [edge_superset]}
                                   # 'roi': roi_prec + [edge_roi]}
                    )
        
    for node1, node2, datas in ret.edges(data=True):
        A = numpy.array(datas.get('superset', None))
        c = Counter(A.flat).most_common(1)
        d = c[0][0]
        ret[node1][node2]['superset']=d
    return ret
```
...
# following correctly raise error
...
```{python}
if not PATIENCE:
    Grainer.experiment(clusters[5:8], metric)
    sizes = np.insert([clusters[5:8]], 0, graph.number_of_nodes())
else:
...
bins = con_distr_bins[0]
hist = con_distr_density[0]
width = alfa * (bins[1] - bins[0])
center = (bins[:-1] + bins[1:]) / 2
ax.bar(center, hist, align='center', width=width)

bins = con_distr_bins[1]
hist = con_distr_density[1]
width = alfa * (bins[1] - bins[0])
center = (bins[:-1] + bins[1:]) / 2
ax.bar(center, hist, align='center', width=width)

bins = con_distr_bins[2]
hist = con_distr_density[2]
width = alfa * (bins[1] - bins[0])
center = (bins[:-1] + bins[1:]) / 2
ax.bar(center, hist, align='center', width=width)

plt.legend(['original', 'iteration 1', 'iteration 2'])
```
