...
weight_metric = np.load('exported-traced-adjacencies/pagerank_computed.pkl', allow_pickle=True)
```

```{python}
metric = 1/weight_metric[0]
for k in weight_metric.keys():
    if k !=0:
        metric = np.vstack( (metric, 1/weight_metric[k]) )
```
...
```{python}
start_time = time.time()
local_pagerank_results = np.zeros((len(neurons_dataframe), len(neurons_dataframe)))
for i in range(len(neurons_dataframe)):
    locality = np.zeros(len(neurons_dataframe))
    locality[i] = 1
    page = pagerank_power(SM, p=0.85, personalize=locality, tol=1e-3)
    local_pagerank_results[i] = page
end_time = time.time()
print("Total time: {:.2f} hours".format((end_time - start_time)/3600))
```

```{python}
np.save("pagerank_computed", local_pagerank_results)
```

```{python}
local_pagerank_results = np.load("pagerank_computed.npy", allow_pickle=True)
print(local_pagerank_results)
```
