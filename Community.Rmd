...
```{python}
roi_dataframe['superset_num'] = [ roi_supersets[i] for i in np.array( links['superset']) ]
```
...
ax.bar(supersets_name, counts/len(parts), color=colors[1:])
ax.set_ylabel('Density')
...
cbar.ax.set_yticklabels(['No link'] + list(roi_supersets.keys()))  # vertically oriented colorbar
cbar.ax.axes.tick_params(length=5)
#ax.set_xticks(np.cumsum(counts))
#ax.set_yticks(np.cumsum(counts))
#ax.set_xticklabels( list(roi_supersets.keys()) )
#ax.set_yticklabels( list(roi_supersets.keys()) )
